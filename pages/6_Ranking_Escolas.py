...
df_uf = df_esc[df_esc["SG_UF"] == uf_sel]

with col2:
    municipios = (df_uf[["CO_MUNICIPIO", "NO_MUNICIPIO"]]
                  .drop_duplicates()
...
df_mun_filtrado = df_uf[df_uf["CO_MUNICIPIO"] == co_mun]
anos_disp = sorted(df_mun_filtrado["ANO"].dropna().unique(), reverse=True)
...
# Localização (defensivo — pode não existir)
...
# ─────────────────────────────────────────────
# PREPARAR DADOS DO ANO
# ─────────────────────────────────────────────
df_ano = df_mun_filtrado[df_mun_filtrado["ANO"] == ano_sel].copy()
...
pct_abaixo_nac = round((n_alerta + n_atencao) / total * 100, 1) if total > 0 else 0.0

if n_alerta > 0:
    st.error(
        f"⚠️ **{n_alerta} escola(s) em Alerta** — IRD abaixo da média de {mun_sel} "
        f"({formatar_br(media_ird_mun, 3)}). Priorize essas unidades no plano de ação."
    )
elif pct_abaixo_nac >= 50:
    st.warning(
        f"🔔 **{pct_abaixo_nac}% das escolas** abaixo da média nacional "
...
st.markdown(
    ...
    f"🟠 Atenção = entre {formatar_br(media_ird_mun, 3)} e {formatar_br(media_ird_nac, 3)} (média nacional) · "
    f"🟢 Favorável = acima de {formatar_br(media_ird_nac, 3)} "
    f"<span class='tooltip-wrap'><span class='info-icon'>i</span>"
    f"<span class='tip'>{TOOLTIP_CLASSIFICACAO}</span></span>",
    unsafe_allow_html=True
)

# Montar colunas de exibição
cols_base  = ["NO_ENTIDADE", "IRD", "RISCO"]
cols_extra = [c for c in ["ICG", "ATU", "AFD", "IED"] if c in df_rank.columns]
if tem_loc:
    df_rank["Localização"] = (
        df_rank["TP_LOCALIZACAO"].map({1: "Urbana", 2: "Rural"}).fillna("—")
    )
...
st.caption("Barras mais à esquerda = menor regularidade — identifique onde estão as escolas críticas")
...
with st.expander("ℹ️ Como interpretar este ranking"):
    ...
    **O que é o IRD?**
    ...
    O valor da escola é a média dos IRDs individuais de todos os seus docentes.

    **Como as escolas são classificadas?**
    A classificação usa as mesmas referências da página **Escola** do RegDoc:
    ...
    **Próximo passo recomendado**
    Identifique as escolas em Alerta e acesse a página **Escola** para ver
    o perfil completo (localização, complexidade) e as orientações específicas.

    **Limitações**
    - O ranking não controla pelo porte, contexto socioeconômico ou complexidade da escola.
    - Escolas pequenas têm IRD mais volátil — a saída de poucos professores
      afeta desproporcionalmente o indicador.
    - Os dados refletem o Censo Escolar do ano de referência selecionado.
...
st.markdown("---")
st.caption(
    "RegDoc · Fonte: Censo Escolar / INEP · "
    "Atualização anual mediante publicação do Censo Escolar."
)
