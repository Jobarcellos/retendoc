...
def classificar_tendencia(df_mun_filtrado, ano_ref):
    ...
    for i in range(1, len(hist)):
        if valores[i] - valores[i-1] <= -0.5:
            ruptura = True
            ano_ruptura = int(hist["ANO"].iloc[i])
            break
...
# ── Indicadores adicionais com tooltip ────────────────────────────────────────
st.markdown("---")
...
with ci1:
    afd_val = formatar_br(linha_atual["AFD"], 1)
    st.markdown(f"""
    <div style="border:1px solid #dde4ed; border-radius:8px; padding:1rem 1.2rem; background:#f7f9fc;">
        <div style="font-size:0.9rem; font-weight:600; color:#1a3a5c; margin-bottom:4px;">
            ...
            <span class="tooltip-wrap">
                ...
                <span class="tip">{TOOLTIPS['AFD']}</span>
            </span>
        </div>
        <div style="font-size:1.6rem; font-weight:bold; color:#1a3a5c;">{afd_val}%</div>
    </div>""", unsafe_allow_html=True)

with ci2:
    ied_val = formatar_br(linha_atual["IED"], 1)
    st.markdown(f"""
    <div style="border:1px solid #dde4ed; border-radius:8px; padding:1rem 1.2rem; background:#f7f9fc;">
        <div style="font-size:0.9rem; font-weight:600; color:#1a3a5c; margin-bottom:4px;">
            Esforço docente (IED)
            <span class="tooltip-wrap">
                <span class="info-icon">i</span>
                <span class="tip">{TOOLTIPS['IED']}</span>
            </span>
        </div>
        <div style="font-size:1.6rem; font-weight:bold; color:#1a3a5c;">{ied_val}%</div>
    </div>""", unsafe_allow_html=True)

with ci3:
    icg_val = formatar_br(linha_atual["ICG"], 1)
    st.markdown(f"""
    <div style="border:1px solid #dde4ed; border-radius:8px; padding:1rem 1.2rem; background:#f7f9fc;">
        <div style="font-size:0.9rem; font-weight:600; color:#1a3a5c; margin-bottom:4px;">
            ...
            <span class="tooltip-wrap">
                <span class="info-icon">i</span>
                <span class="tip">{TOOLTIPS['ICG']}</span>
            </span>
        </div>
        <div style="font-size:1.6rem; font-weight:bold; color:#1a3a5c;">{icg_val}</div>
    </div>""", unsafe_allow_html=True)
...
<div class="header">
  ...
  <div style="font-size:0.8rem;color:#b8cfe8;text-align:right;">
...
<div class="section">
  ...
  <table>
    <thead><tr><th>Ano</th><th>Regularidade</th><th>Variação</th>
