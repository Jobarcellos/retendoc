...
def render_cruzamento_saeb(dados_comp, df_esc):
    from utils.dados import carregar_saeb
    
    st.markdown("---")
    st.markdown("### Desempenho dos alunos × Regularidade dos professores")
    st.caption(
        "Cruzamento entre o IRD da escola e a proficiência média no Saeb "
        "(2019, 2021 e 2023). Escolas com maior regularidade docente "
        "tendem a apresentar melhores resultados de aprendizagem."
    )

    df_saeb = carregar_saeb()

    # Montar base de cruzamento
    registros = []
    for d in dados_comp:
        co = d["CO_ENTIDADE"]
        ird = d["IRD"]
        if pd.isna(ird):
            continue
        saeb_esc = df_saeb[df_saeb["CO_ENTIDADE"] == str(co)]
        if saeb_esc.empty:
            continue
        for _, row in saeb_esc.iterrows():
            if pd.notna(row["MEDIA_GERAL"]):
                registros.append({
                    "Escola": d["Escola"],
                    "Município": d["Município"],
                    "UF": d["UF"],
                    "IRD": ird,
                    "Proficiência média": row["MEDIA_GERAL"],
                    "LP 5º EF": row.get("MEDIA_5EF_LP"),
                    "MT 5º EF": row.get("MEDIA_5EF_MT"),
                    "LP 9º EF": row.get("MEDIA_9EF_LP"),
                    "MT 9º EF": row.get("MEDIA_9EF_MT"),
                    "Nível socioeconômico": row.get("NIVEL_SOCIO_ECONOMICO"),
                    "Ano Saeb": int(row["ANO_SAEB"]),
                })

    if not registros:
        st.warning(
            "Nenhuma das escolas selecionadas tem dados do Saeb disponíveis. "
            "O Saeb cobre principalmente escolas com Ensino Fundamental — "
            "escolas de EI, EJA ou EM puro podem não aparecer."
        )
        return

    df_cruz = pd.DataFrame(registros)

    # ── Gráfico de dispersão IRD × Proficiência ───────────────────────────────
    cores_escolas = ["#1a3a5c","#c0392b","#27ae60","#f39c12","#8e44ad"]
    fig = go.Figure()

    for i, escola in enumerate(df_cruz["Escola"].unique()):
        df_e = df_cruz[df_cruz["Escola"] == escola]
        cor  = cores_escolas[i % len(cores_escolas)]
        fig.add_trace(go.Scatter(
            x=df_e["IRD"],
            y=df_e["Proficiência média"],
            mode="markers+text",
            name=escola,
            marker=dict(size=14, color=cor, opacity=0.85),
            text=df_e["Ano Saeb"].astype(str),
            textposition="top center",
            textfont=dict(size=10),
            hovertemplate=(
                "<b>%{customdata[0]}</b><br>"
                "Ano Saeb: %{customdata[1]}<br>"
                "IRD: %{x:.2f}<br>"
                "Proficiência média: %{y:.1f}<br>"
                "Nível socioeconômico: %{customdata[2]}<extra></extra>"
            ),
            customdata=df_e[["Escola","Ano Saeb","Nível socioeconômico"]].values,
        ))

    fig.update_layout(
        height=450,
        margin=dict(l=20,r=20,t=40,b=20),
        xaxis=dict(title="Regularidade dos professores — IRD (0 a 5)", range=[0,5.2]),
        yaxis=dict(title="Proficiência média Saeb"),
        legend=dict(orientation="h", y=-0.2),
    )
    st.plotly_chart(fig, use_container_width=True)

    # ── Tabela detalhada ──────────────────────────────────────────────────────
    with st.expander("Ver dados detalhados do Saeb por escola"):
        df_tab = df_cruz[[
            "Escola","Município","UF","Ano Saeb","IRD",
            "LP 5º EF","MT 5º EF","LP 9º EF","MT 9º EF",
            "Proficiência média","Nível socioeconômico"
        ]].copy()
        for col in ["IRD","LP 5º EF","MT 5º EF","LP 9º EF","MT 9º EF","Proficiência média"]:
            df_tab[col] = df_tab[col].apply(
                lambda x: formatar_br(x,1) if pd.notna(x) else "—"
            )
        st.dataframe(df_tab, use_container_width=True, hide_index=True)

    # ── Nota metodológica ─────────────────────────────────────────────────────
    st.caption(
        "⚠️ **Limitação importante:** a comparação IRD × Saeb não controla pelo "
        "nível socioeconômico (INSE) das escolas. Escolas em contextos mais "
        "vulneráveis tendem a ter proficiência menor independentemente do IRD. "
