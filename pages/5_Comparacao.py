...
# ── OPÇÃO 1 — Comparação lado a lado ─────────────────────────────────────────
st.markdown("---")
...
for i, d in enumerate(dados_comp):
    ...
    with cols_cards[i]:
        ...
        <div style="background:{bg}; border:2px solid {cor}; border-radius:10px;
            ...
            <p style="font-size:0.65rem; color:{cor}; margin:0.2rem 0;">
                Regularidade dos professores (0 a 5)
                <span class="tooltip-wrap">
                    <span class="info-icon">i</span>
                    <span class="tip">O IRD mede se os mesmos professores continuam na escola de um ano para o outro. Calculado pelo Inep a partir do Censo Escolar. Escala de 0 a 5 — quanto maior, mais estável é o corpo docente.</span>
                </span>
            </p>
        ...
        <div style="background:#f7f9fc; border-radius:8px; padding:0.8rem; font-size:0.8rem;">
            <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                <span>Alunos/turma</span><strong>{formatar_br(d['ATU'],1)}</strong>
            </div>
            <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                <span>Formação (%)</span><strong>{formatar_br(d['AFD'],1)}{'%' if pd.notna(d['AFD']) else ''}</strong>
            </div>
            <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                <span>Esforço docente</span><strong>{formatar_br(d['IED'],1)}{'%' if pd.notna(d['IED']) else ''}</strong>
            </div>
            <div style="display:flex; justify-content:space-between;">
                <span>Complexidade</span><strong>{formatar_br(d['ICG'],1)}</strong>
            </div>
        </div>
        """, unsafe_allow_html=True)

# ── OPÇÃO 2 — Comparação com média nacional ───────────────────────────────────
st.markdown("---")
...
# Gráfico radar — todos os indicadores
st.markdown("### Perfil completo dos indicadores")
st.caption("Quanto maior a área, melhor o perfil geral da escola")

categorias = ["Regularidade (IRD)", "Alunos/turma (invertido)", "Formação (%)", "Esforço (invertido)", "Complexidade"]
...
st.caption("Valores normalizados 0-100. ATU e Esforço docente estão invertidos — valores mais altos indicam situação mais favorável.")

# ── OPÇÃO 3 — Ranking de escolas do município ─────────────────────────────────
st.markdown("---")
...
df_rank_mun = df_esc[
    (df_esc["CO_MUNICIPIO"] == co_mun_rank) &
    (df_esc["ANO"] == ano_rank)
...
escolas_destacadas = [d["CO_ENTIDADE"] for d in dados_comp
...
for _, row in df_rank_mun.iterrows():
    if row["CO_ENTIDADE"] in escolas_destacadas:
        cores_rank.append("#1a3a5c")
    elif row["IRD"] < media_nac_rank * 0.85:
        cores_rank.append("#c0392b")
    elif row["IRD"] < media_nac_rank:
        cores_rank.append("#e67e22")
    elif row["IRD"] < media_nac_rank * 1.1:
        cores_rank.append("#f1c40f")
    else:
        cores_rank.append("#27ae60")

nomes_curtos = [n[:20] + "..." if len(n) > 20 else n for n in df_rank_mun["NO_ENTIDADE"]]

fig_rank.add_trace(go.Bar(
    x=df_rank_mun["IRD"].tolist(),
    y=nomes_curtos,
    orientation="h",
...
st.caption("Escolas destacadas em azul escuro são as que você adicionou para comparação. Vermelho = Alerta | Laranja = Atenção | Amarelo = Moderado | Verde = Favorável.")
...
df_tab = df_tab.rename(columns={
    "Nome completo": "Escola", "Município": "Município", "UF": "UF",
    "Ano": "Ano", "IRD": "Regularidade", "ATU": "Alunos/turma",
    "AFD": "Formação (%)", "IED": "Esforço docente (%)", "ICG": "Complexidade"
})
